...
- **`pf` and `gateways` measurements can silently stop reporting if Telegraf runs as an unprivileged user.** The `pkg install telegraf` daemon runs as the `telegraf` system user, not root. That user has no default access to `/dev/pf` (needed by `inputs.pf`) or `/conf/config.xml` (needed by `telegraf_pfifgw.php`, which reads it via OPNsense's `Config.php`), so both silently fail on every gather cycle: `inputs.pf` logs `pfctl: /dev/pf: Permission denied`, and the exec plugin logs a PHP fatal error (`flock(): Argument #1 ($stream) must be of type resource, false given`) from `Config.php`. Every other measurement (`cpu`, `mem`, `disk`, `net`, `system`, `temperature`) keeps working, so the dashboard looks mostly fine — except the `$Host` variable, which is built from `pf`, so it resolves to nothing and every panel that filters on it (CPU per-core, RAM, Load, Temperature, Disk, LAN, Gateway) shows "No data" too, even though those measurements are actually fine. Fix, run on the router as root:
  ```sh
  pw groupmod proxy -m telegraf
  setfacl -m u:telegraf:r-----a-R-c--s::allow /conf/config.xml
  service telegraf restart
  ```
  The first line grants `/dev/pf` access via its existing group. The second grants read-only access to just `config.xml` via a scoped NFSv4 ACL (requires ZFS/UFS with ACLs enabled) instead of adding `telegraf` to `wheel`, which would hand a monitoring daemon read access to every secret on the box. Check `/var/log/telegraf/telegraf.log` for `E! [inputs.pf]` / `E! [inputs.exec]` lines to confirm this is what you're hitting before applying it.
